...
export function useScrollHeightTracker() {
  ...
  function elementIsInViewport() {
    ...
    const parentDistanceFromTop = Math.round(
      parentMeasurements.top + window.scrollY,
    );
    const childDistanceFromTop = Math.round(
      childMeasurements.top + window.scrollY,
    );

    if (childDistanceFromTop - 10 >= parentDistanceFromTop) {
      calculateAndSetSectionNumber(
        parentDistanceFromTop,
        childDistanceFromTop,
        parentMeasurements,
        childMeasurements,
      );
      calculateAndSetElemPosition(
        parentDistanceFromTop,
        childDistanceFromTop,
        childMeasurements,
      );
  ...
  function calculateAndSetSectionNumber(
    parentDistanceFromTop,
    childDistanceFromTop,
    parentMeasurements,
    childMeasurements,
  ) {
    var childPos = Math.round(
      childDistanceFromTop + childMeasurements.height - parentDistanceFromTop,
    );
    var numOfSections = Math.ceil(
      parentMeasurements.height / childMeasurements.height,
    );

    if (
      parentMeasurements.height - 50 <= childPos &&
      sectionNumber !== numOfSections
    ) {
      ...
      for (let i = 1; i < numOfSections; i++) {
        ...
        if (
          childPos >= section - 50 &&
          childPos <= section + (childMeasurements.height - 50)
        ) {
  ...
  function calculateAndSetElemPosition(
    parentDistanceFromTop,
    childDistanceFromTop,
    childMeasurements,
  ) {
