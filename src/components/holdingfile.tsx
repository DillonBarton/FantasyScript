...
import styles from "../src/styles/glassCarousel.module.css";

import {
  SidepanelContainerKF,
  CenterPanelContainerKF,
  PanelKF,
  SidePanelKF,
} from "../src/styled-components/Keyframes";
...
export default function GlassCarousel() {
  ...
  const Panel = styled.div`
    ...
    ${({ panelPositon }) =>
      startAnimation
        ? handlePanelAnimation(panelPositon)
        : handlePanelPosition(panelPositon)}
  `;
  ...
  return (
    <div id="center" className="flexColumn sc boxW100" style={{ zIndex: 100 }}>
      <HeroSection className="boxW100">
        <PanelWrapper
          wrapperPosition={snarr.current[1]}
          className={`${snarr.current[1] ? handleWrapperPosition(snarr.current[1]) : handleWrapperPosition(snarr.current[1])} boxH100`}
        >
          <PanelContainer
            containerPositon={snarr.current[1]}
            className="boxH100"
          >
            <Panel
              panelPositon={snarr.current[1]}
              className="flexColumn sc boxH100"
            >
              <PanelMediaContainer>
        ...
        <PanelWrapper
          wrapperPosition={snarr.current[2]}
          className={`${snarr.current[2] ? handleWrapperPosition(snarr.current[2]) : handleWrapperPosition(snarr.current[2])} boxW100H100`}
        >
          <PanelContainer
            containerPositon={snarr.current[2]}
            className="boxW100H100"
          >
            <Panel
              panelPositon={snarr.current[2]}
              className="flexColumn sc boxW100H100"
            >
        ...
        <PanelWrapper
          wrapperPosition={snarr.current[3]}
          className={`${snarr.current[3] ? handleWrapperPosition(snarr.current[3]) : handleWrapperPosition(snarr.current[3])} boxH100`}
        >
          <PanelContainer
            containerPositon={snarr.current[3]}
            className="boxH100"
          >
            <Panel
              panelPositon={snarr.current[3]}
              className="flexColumn sc boxH100"
            >
              <PanelMediaContainer>
                <img
                  src="/images/responsiveDesign.webp"
