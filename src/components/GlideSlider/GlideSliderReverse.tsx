...
export default function GlideSliderReverse({unitOfMeasurement, duration, width}){
    ...
    function setCloneNodes(){
        let firstSliderClone = sliderTrackReverse.current.childNodes[sliderTrackReverse.current.childNodes.length - 2].cloneNode(true);
        let secondSliderClone = sliderTrackReverse.current.childNodes[sliderTrackReverse.current.childNodes.length - 1].cloneNode(true);
        sliderTrackReverse.current.prepend(firstSliderClone, secondSliderClone)
    }

    function getFullDistance(){
        let nodeListLengthMinusClones = sliderTrackReverse.current.childNodes.length - 2;
    ...
    function getDuration(fullDistance){
        ...
        return Math.abs(sliderTrackReverse.current.getBoundingClientRect().x) / speed;
    }
    ...
    useEffect(()=>{
        ...
        sliderTrackReverse.current.addEventListener("transitionend", resetAnimation);
    ...
    return(
        <div className={`${styles.sliderContainer} boxW100`}>
            ...
            <div ref={sliderTrackReverse} id="sliderTrackReverse" className={`${styles.sliderTrackReverse} boxW100H100 flexRow sc`}>
