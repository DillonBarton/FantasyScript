  ...
  const validityChecker = (regexp, value, inputFunc, minLength, maxLength) => {
    ...
    if (len >= minLength && len <= maxLength) {
      if (regex.test(value)) {
        ...
        console.log(
          "Your full name must contain no numbers or special characters",
        );
        inputFunc("inValid");
      }
    } else {
      console.log(
        `Your full name must be between ${minLength} and ${maxLength} characters`,
      );
      inputFunc("inValid");
      if (value === "") {
        inputFunc("loading");
  ...
  return (
    <section className={`${styles.sectionFormContainer} flexRow cc`}>
      <div className={`${styles.formWrapper} flexRow cc`}>
        <form
          id="applicantForm"
          className={`${styles.applicantForm} flexRow sc`}
        >
          <div className={`${styles.inputFieldsContainer} flexRow sc`}>
            <div className={`${styles.inputContainer} flexRow sc`}>
              ...
              />
              <div
                className={`${styles.inputCover} ${fullName === "valid" ? styles.validInput : fullName === "inValid" ? styles.inValidInput : fullName === null ? styles.nullInput : null}`}
              >
                {fullName === null ? (
            ...
            <div className={`${styles.inputContainer} flexRow sc`}>
              ...
              />
              <div
                className={`${styles.inputCover} ${companyName === "valid" ? styles.validInput : companyName === "inValid" ? styles.inValidInput : companyName === null ? styles.nullInput : null}`}
              >
                {companyName === null ? (
                  <FaArrowLeft width="30px" height="30px" color="#e6e6e6" />
                ) : companyName === "loading" ? (
                  <LoaderSVG
                    strokeWidth="4"
                    colour="#e6e6e6"
          ...
          <div className={`${styles.inputFieldsContainer} flexRow sc`}>
            <div className={`${styles.inputContainer} flexRow sc`}>
              ...
              />
              <div
                className={`${styles.inputCover} ${email === "valid" ? styles.validInput : email === "inValid" ? styles.inValidInput : email === null ? styles.nullInput : null}`}
              >
                {email === null ? (
                  <FaArrowLeft width="30px" height="30px" color="#e6e6e6" />
                ) : email === "loading" ? (
            ...
            <div className={`${styles.inputContainer} flexRow sc`}>
              ...
              />
              <div
                className={`${styles.inputCover} ${number === "valid" ? styles.validInput : number === "inValid" ? styles.inValidInput : number === null ? styles.nullInput : null}`}
              >
                {number === null ? (
                  <FaArrowLeft width="30px" height="30px" color="#e6e6e6" />
                ) : number === "loading" ? (
          ...
          <div className={`${styles.inputFieldsContainer} flexRow sc`}>
            <div
              className={`${styles.inputContainer} ${styles.description}  ${projectDescription === "valid" ? styles.validInput : projectDescription === "inValid" ? styles.inValidInput : projectDescription === null ? styles.nullInput : null} flexRow sc`}
            >
              {projectDescription === null ? (
              ...
              <textarea
                ...
                maxLength={275}
              ></textarea>
              <div className={styles.inputCover}></div>
            </div>
          </div>
          <div
            className={`${styles.inputFieldsContainer} flexRow sc`}
          ...
          >
            <div className={`${styles.fileInputContainer} flexRow sc`}>
              ...
              <input
                ...
                type="file"
                name="projectFiles"
                accept="image/* .doc, .docx, .pdf, .ppt, .pptx, .xml, application/msword, application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                capture
                multiple
              />
            </div>
          ...
          <div className={`${styles.formInformation} boxW100`}>
            Accepted file types: .doc, .docx, .pdf, .ppt, .pptx, .xml, .png,
            .jpg, .jpeg etc...
          </div>
          <div className="boxW100 flexRow ec">
            <button
              form="applicantForm"
              className={`${styles.submitButton} flexRow cc`}
              type="submit"
            >
              <div>Submit</div>
