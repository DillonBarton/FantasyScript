...
export default function PageSection({
  ...
  return (
    <div className={`${styles.wrapper} flexRow flexGrow cc boxW100H100`}>
      <div
        className={`${styles.display} ${isReversed ? styles.order2 : styles.order1} flexRow sc boxH100`}
      >
        {children}
      </div>

      <div
        className={`${styles.textContainer} ${isReversed ? styles.order1 : styles.order2} flexColumn cs boxH100`}
      >
        <div
          ref={sectionRef}
          className={`${styles.text} flexColumn cs boxW100`}
        >
          <h2 className={`${styles.textHeader} boxW100`}>{title}</h2>
          <h4 className={`${styles.textHeader} boxW100`}>{subtitle}</h4>
